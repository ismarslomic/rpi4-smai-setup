...
# SSH enabled by using (undocumented) scripts of raspi-config command,
# see https://github.com/RPi-Distro/raspi-config/blob/master/raspi-config

function main() {
  e_header "Enabling SSH"

  # 0 = enabled, 1 = disabled
  sudo raspi-config nonint do_ssh 0
  ip=$(hostname -I)

  e_note "Connect with SSH to your Rpi by executing 'ssh pi@$ip'"
  e_success "Enabling SSH completed"
}
